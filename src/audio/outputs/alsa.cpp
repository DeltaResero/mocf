                        ...
                        double *min_norm, bool *use_linear_volume_scale)
{
  ...
  {
    res = (exp10((real_vol - *mixer_max) / 6000.0) - *min_norm) * 100 /
          (1 - *min_norm);
  }
                                 ...
                                 bool *use_linear_volume_scale)
{
  ...
  {
    res = lrint(6000.0 * log10(vol * (1 - *min_norm) / 100 + *min_norm) +
                *mixer_max);
  }
...
static const struct
{
  ...
  long format;
} format_masks[] = {{SND_PCM_FORMAT_S8, SFMT_S8},
...
static snd_pcm_hw_params_t *alsa_open_device(const char *device)
{
  ...
  rc = snd_pcm_open(&handle, device, SND_PCM_STREAM_PLAYBACK, SND_PCM_NONBLOCK);
  if (rc < 0)
  {
...
static int fill_capabilities(struct output_driver_caps *caps)
{
  ...
  do
  {
    ...
    if (rc < 0)
    {
      error_errno("Can't restrict ALSA configuration space to contain only "
                  "real hardware rates",
                  rc);
      break;
    }

    rc = snd_pcm_hw_params_get_rate_min(hw_params, &val, nullptr);
    if (rc < 0)
    {
    ...
    rc = snd_pcm_hw_params_get_rate_max(hw_params, &val, nullptr);
    if (rc < 0)
    {
                                                 ...
                                                 bool *use_linear_volume_scale)
{
  ...
  else
  {
    ...
    if ((snd_mixer_selem_get_playback_dB_range(elem, &db_min, &db_max) >= 0) &&
        (db_min < db_max) && (db_max - db_min >= MAX_LINEAR_DB_SCALE))
    {
      ...
      *vol_max = db_max;
      logit(
          "Opened mixer (%s), perceptive volume range: %ld-%lddB, min_norm: %f",
          name, *vol_min, *vol_max, *min_norm);
    }
    ...
    {
      logit("Opened mixer (%s), linear volume range: %ld-%ld", name, *vol_min,
            *vol_max);
    }
  }
...
static void alsa_open_mixer(const char *device)
{
  ...
  rc = snd_mixer_selem_register(mixer_handle, nullptr, nullptr);
  if (rc < 0)
  {
...
static int alsa_init(struct output_driver_caps *caps)
{
  ...
  if (mixer_handle)
  {
    mixer_elem1 = alsa_init_mixer_channel(options_get_str("ALSAMixer1"),
                                          &mixer1_min, &mixer1_max, &min_norm1,
                                          &use_linear_volume_scale1);
    mixer_elem2 = alsa_init_mixer_channel(options_get_str("ALSAMixer2"),
                                          &mixer2_min, &mixer2_max, &min_norm2,
                                          &use_linear_volume_scale2);
  }
...
static int alsa_open(struct sound_params *sound_params)
{
  ...
  rc = snd_pcm_hw_params_set_rate_near(handle, hw_params, &params.rate, nullptr);
  if (rc < 0)
  {
  ...
  rc =
      snd_pcm_hw_params_set_channels(handle, hw_params, sound_params->channels);
  if (rc < 0)
  {
  ...
  rc = snd_pcm_hw_params_get_buffer_time_max(hw_params, &buffer_time, nullptr);
  if (rc < 0)
  {
  ...
  rc = snd_pcm_hw_params_set_buffer_time_near(handle, hw_params, &buffer_time,
                                              nullptr);
  if (rc < 0)
  {
  ...
  snd_pcm_hw_params_get_period_size(hw_params, &chunk_frames, nullptr);
  ...
  {
    error("Can't use period equal to buffer size (%lu == %lu)", chunk_frames,
          buffer_frames);
    goto err;
  }
...
static int alsa_read_mixer()
{
  ...
  if (mixer_elem_curr == mixer_elem1)
  {
    ...
    vol = &volume1;
    curr_real_vol =
        alsa_read_mixer_raw(mixer_elem_curr, use_linear_volume_scale1);
  }
  else
  {
    ...
    vol = &volume2;
    curr_real_vol =
        alsa_read_mixer_raw(mixer_elem_curr, use_linear_volume_scale2);
  }
  ...
  if (*real_vol != curr_real_vol)
  {
    ...
    *vol = (vol == &volume1)
               ? scale_volume(*real_vol, &mixer1_min, &mixer1_max, &min_norm1,
                              &use_linear_volume_scale1)
               : scale_volume(*real_vol, &mixer2_min, &mixer2_max, &min_norm2,
                              &use_linear_volume_scale2);
...
static void alsa_set_mixer(int vol)
{
  if (mixer_handle && mixer_elem_curr)
  {
    ...
    if (mixer_elem_curr == mixer_elem1)
    {
      volume1 = vol;
      real_vol = &real_volume1;
      vol_alsa = scale_volume_inverse(volume1, &mixer1_min, &mixer1_max,
                                      &min_norm1, &use_linear_volume_scale1);

      debug("Setting vol to %ld, requested: %d", vol_alsa, vol);

      rc = use_linear_volume_scale1
               ? snd_mixer_selem_set_playback_volume_all(mixer_elem_curr,
                                                         vol_alsa)
               : snd_mixer_selem_set_playback_dB_all(mixer_elem_curr, vol_alsa,
                                                     0);
    ...
    else
    {
      volume2 = vol;
      real_vol = &real_volume2;
      vol_alsa = scale_volume_inverse(volume2, &mixer2_min, &mixer2_max,
                                      &min_norm2, &use_linear_volume_scale2);

      debug("Setting vol to %ld, requested: %d", vol_alsa, vol);

      rc = use_linear_volume_scale2
               ? snd_mixer_selem_set_playback_volume_all(mixer_elem_curr,
                                                         vol_alsa)
               : snd_mixer_selem_set_playback_dB_all(mixer_elem_curr, vol_alsa,
                                                     0);
