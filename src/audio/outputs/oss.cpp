...
static int oss_init(struct output_driver_caps *caps)
{
  ...
  {
    mixer_channel1 =
        oss_mixer_name_to_channel(options_get_symb("OSSMixerChannel1"));
    mixer_channel2 =
        oss_mixer_name_to_channel(options_get_symb("OSSMixerChannel2"));
...
static int oss_set_params()
{
  ...
  logit("Audio parameters set to: %s, %d channels, %dHz",
        sfmt_str(params.fmt, fmt_name, sizeof(fmt_name)), params.channels,
        params.rate);
