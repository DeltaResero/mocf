...
#include "core/log.h"
#include "audio/decoders/mp3/xing.h"
...
static std::string get_tag(struct id3_tag *tag, const char *what)
{
  ...
  if (frame)
  {
    ...
    if (ucs4)
    {
      ...
      {
        std::unique_ptr<char, decltype(&::free)> latin1(
            reinterpret_cast<char *>(id3_ucs4_latin1duplicate(ucs4)), &::free);
        if (latin1)
        {
          result = id3v1_fix(latin1.get());
        }
      }
      else
      {
...
static int count_time_internal(struct mp3_data *data)
{
  ...
  */
  ...
  while (true)
  {
    ...
    if (!num_frames++)
    {
      if (xing_parse(&xing, data->stream.anc_ptr, data->stream.anc_bitlen) !=
          -1)
      {
        is_vbr = 1;
  ...
  {
    data->avg_bitrate =
        data->size / mad_timer_count(duration, MAD_UNITS_SECONDS) * 8;
  }
  ...
  while (true)
  {
    ...
    if (mad_frame_decode(&data->frame, &data->stream))
    {
      ...
      else if (MAD_RECOVERABLE(data->stream.error))
      {
        ...
        if (!data->skip_frames)
        {
          decoder_error(&data->error, ERROR_STREAM, 0, "Broken frame: %s",
      ...
      {
        decoder_error(&data->error, ERROR_FATAL, 0, "Broken frame: %s",
                      mad_stream_errorstr(&data->stream));
        return 0;
      }
    }
    ...
    return put_output(buf, buf_len, &data->synth.pcm, &data->frame.header);
  }
}
...
{
  return !strcasecmp(ext, "mp3") || !strcasecmp(ext, "mpga") ||
         !strcasecmp(ext, "mp2") || !strcasecmp(ext, "mp1");
}
