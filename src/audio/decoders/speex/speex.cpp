...
static int read_speex_header(struct spx_data *data)
{
  ...
  while (packet_count < header_packets)
  {
    ...
    if (nb_read == 0)
    {
      decoder_error(&data->error, ERROR_FATAL, 0, "Can't open speex header");
      return 0;
    }
    ...
    while (ogg_sync_pageout(&data->oy, &data->og) == 1)
    {
      ...
      while (ogg_stream_packetout(&data->os, &data->op) == 1)
      {
        ...
        if (packet_count == 0)
        {
          ...
          data->nchannels = data->header->nb_channels;
          data->frames_per_packet = data->header->frames_per_packet;
                      ...
                      struct sound_params *sound_params)
{
  ...
  while (nbytes)
  {
    ...
    else if (ogg_stream_packetout(&data->os, &data->op) == 1)
    {
      ...
      {
        /* Decode frame */
        speex_decode_int(data->st, &data->bits, temp_output);
        if (data->nchannels == 2)
        {
          speex_decode_stereo_int(temp_output, data->frame_size, &data->stereo);
        }

        speex_decoder_ctl(data->st, SPEEX_GET_BITRATE, &data->bitrate);
        /*data->samples_decoded += data->frame_size;*/

        temp_output += data->frame_size * data->nchannels;
      }
      ...
      data->output_start = 0;
      data->output_left =
          data->frame_size * data->nchannels * data->frames_per_packet;
    }
...
extern "C" class AudioPlugin *speex_plugin_init() {
