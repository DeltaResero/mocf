                          ...
                          va_list vl)
{
  ...
  msg = format_msg_va(fmt, vl);
    ...
    LIBAVFORMAT_VERSION_INT >= AV_VERSION_INT(56, 33, 101)
...
static void ffmpeg_info(const char *file_name, struct file_tags *info,
                        const int tags_sel)
{
  int err;
...
#else
  // option types is a string - see AV_OPT_TYPE_CHLAYOUT handling in
ffmpeg:libavutil/opt.c char downmix_layout[64];

  if (data->enc->ch_layout.nb_channels <= 2)
    return;
  av_channel_layout_describe(&data->enc->ch_layout, downmix_layout,
sizeof(downmix_layout)); av_dict_set(&data->opts, "downmix", downmix_layout, 0);
#endif
}
*/
...
static void *ffmpeg_open_internal(struct ffmpeg_data *data)
{
  ...
  if (err < 0)
  {
    decoder_error(&data->error, ERROR_FATAL, 0, "Failed to copy codec context");
    goto end;
  }
#endif
...
static int decode_audio(AVCodecContext *ctx, AVFrame *frame, int *got_frame_ptr,
                        const AVPacket *pkt)
{
  int rc, result = 0;
...
static int decode_packet(struct ffmpeg_data *data, AVPacket *pkt, char *buf,
                         int buf_len)
{
  int filled = 0;
  char *packed;
