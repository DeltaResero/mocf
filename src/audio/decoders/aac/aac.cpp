                                          ...
                                          const char *fname)
{
  ...
  {
    decoder_error(&data->error, ERROR_FATAL, 0, "AAC file/stream too short");
    return data;
  }
  ...
  {
    decoder_error(&data->error, ERROR_FATAL, 0, "Invalid AAC sound parameters");
    return data;
  }
...
static void aac_info(const char *file_name, struct file_tags *info,
                     const int tags_sel)
{
...
static int decode_one_frame(struct aac_data *data, void *buffer, int count)
{
  ...
  data->bitrate = frame_info.bytesconsumed * 8 /
                  (bytes / 2.0 / data->channels / data->sample_rate) / 1000;
