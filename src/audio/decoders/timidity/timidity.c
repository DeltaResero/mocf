...
static struct timidity_data *make_timidity_data(const char *file)
{
  ...
  if (midistream == NULL)
  {
    decoder_error(&data->error, ERROR_FATAL, 0, "Can't open midifile: %s",
                  file);
    return data;
  }
  ...
  {
    decoder_error(&data->error, ERROR_FATAL, 0, "Can't load midifile: %s",
                  file);
    return data;
  }
                           ...
                           struct sound_params *sound_params)
{
  ...
  sound_params->fmt =
      (midioptions.format == MID_AUDIO_S16LSB) ? (SFMT_S16 | SFMT_LE) : SFMT_S8;

  return mid_song_read_wave(data->midisong, (void *)buf, buf_len);
}

static int timidity_get_bitrate(void *unused ATTR_UNUSED) { return -1; }
...
static void timidity_destroy() { mid_exit(); }

static struct decoder timidity_decoder = {DECODER_API_VERSION,
                                          NULL,
                                          timidity_destroy,
                                          timidity_open,
                                          timidity_close,
...
struct decoder *timidity_plugin_init()
{
  ...
  midioptions.format = (options_get_int("TiMidity_Bits") == 16)
                           ? MID_AUDIO_S16LSB
                           : MID_AUDIO_S8;
