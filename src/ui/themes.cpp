...
static void set_default_colors()
{
  ...
  make_color(CLR_MENU_ITEM_DIR_SELECTED, COLOR_BLACK, COLOR_CYAN, A_BOLD);
  make_color(CLR_MENU_ITEM_PLAYLIST, COLOR_WHITE, COLOR_BLACK, A_BOLD);
  make_color(CLR_MENU_ITEM_PLAYLIST_SELECTED, COLOR_BLACK, COLOR_CYAN, A_BOLD);
  make_color(CLR_MENU_ITEM_FILE, COLOR_WHITE, COLOR_BLACK, A_NORMAL);
  make_color(CLR_MENU_ITEM_FILE_SELECTED, COLOR_BLACK, COLOR_WHITE, A_NORMAL);
  make_color(CLR_MENU_ITEM_FILE_MARKED, COLOR_YELLOW, COLOR_BLACK, A_BOLD);
  make_color(CLR_MENU_ITEM_FILE_MARKED_SELECTED, COLOR_YELLOW, COLOR_WHITE,
             A_BOLD);
  ...
  make_color(CLR_MENU_ITEM_INFO_MARKED_SELECTED, COLOR_YELLOW, COLOR_WHITE,
             A_BOLD);
                               ...
                               const int errors_are_fatal)
{
  ...
  if (!(foreground = strtok(nullptr, " \t")))
  {
    if (errors_are_fatal)
    {
      theme_parse_error(line_num, "foreground color not specified");
    }
    return 0;
  }
  if (!(background = strtok(nullptr, " \t")))
  {
    if (errors_are_fatal)
    {
      theme_parse_error(line_num, "background color not specified");
    }
    return 0;
  }
  if ((attributes = strtok(nullptr, " \t")))
  {
    ...
    if ((tmp = strtok(nullptr, " \t")))
    {
      if (errors_are_fatal)
      {
        theme_parse_error(line_num, "unexpected chars at the end of line");
      }
      return 0;
    }
    ...
    do
    {
      ...
      else
      {
        if (errors_are_fatal)
        {
          theme_parse_error(line_num, "unknown attribute");
        }
        return 0;
      }
  ...
  if ((clr_fore = find_color_name(foreground)) == -1)
  {
    if (errors_are_fatal)
    {
      theme_parse_error(line_num, "bad foreground color name");
    }
    return 0;
  }
  if ((clr_back = find_color_name(background)) == -1)
  {
    if (errors_are_fatal)
    {
      theme_parse_error(line_num, "bad background color name");
    }
    return 0;
  }
                                   ...
                                   const int errors_are_fatal, char *tmp)
{
  ...
  if (*end)
  {
    if (errors_are_fatal)
    {
      theme_parse_error(line_num, "color value is not a valid number");
    }
    return -1;
  }
  if (!RANGE(0, color, 1000))
  {
    if (errors_are_fatal)
    {
      theme_parse_error(line_num, "color value should be in range 0-1000");
    }
    return -1;
  }
 ...
 * theme_parse_error() is invoked, otherwise 0 is returned. */
static int parse_theme_colordef(const int line_num, const int errors_are_fatal)
{
  char *name;
  char *tmp, *tmp2;
  ...
  if (tmp[0] == '#')
  {
    ...
    if (*end)
    {
      if (errors_are_fatal)
      {
        theme_parse_error(line_num, "expected '#RRGGBB'");
      }
      return -1;
    }
...
int get_color(const enum color_index index) { return colors[index]; }
