...
#define STARTUP_MESSAGE                                                        \
  "Welcome to " PACKAGE_NAME " (version " PACKAGE_VERSION ")!"
#else
#define STARTUP_MESSAGE                                                        \
  "Welcome to " PACKAGE_NAME " (version " PACKAGE_VERSION                      \
...
static void entry_add_char(struct entry *e, const wchar_t c)
{
  ...
  memmove(e->text_ucs + e->cur_pos + 1, e->text_ucs + e->cur_pos,
...
static void entry_del_chars(struct entry *e, int count)
{
  ...
  memmove(e->text_ucs + e->cur_pos - count, e->text_ucs + e->cur_pos,
...
static void update_menu_item(struct menu_item *mi, const struct plist *plist,
...
static int side_menu_update_item(struct side_menu *m, const struct plist *plist,
                                 const int n)
{
  struct menu_item *mi;
  int visible = 0;
...
static void main_win_set_dir_content(struct main_win *w,
                                     const enum iface_menu iface_menu,
                                     const struct plist *files,
                                     const lists_t_strs *dirs,
                                     const lists_t_strs *playlists)
{
...
static void main_win_set_played_file(struct main_win *w, const char *file)
{
  ...
  for (ix = 0; ix < ARRAY_SIZE(w->menus); ix += 1)
  {
    ...
    if (m->visible && (m->type == MENU_DIR || m->type == MENU_PLAYLIST))
    {
...
static void info_win_init(struct info_win *w)
{
  ...
  bar_init(&w->time_bar, COLS - 4, "", 1,
           ...
           get_color(CLR_TIME_BAR_FILL), get_color(CLR_TIME_BAR_EMPTY));
}
...
static void info_win_tick(struct info_win *w) { info_win_display_msg(w); }
                                      ...
                                      const struct iface_key *k)
{
  ...
  if (type == ENTRY_SEARCH)
  {
    ...
    else
    {
      ...
      if (cmd == KEY_CMD_MENU_UP || cmd == KEY_CMD_MENU_DOWN ||
          cmd == KEY_CMD_MENU_NPAGE || cmd == KEY_CMD_MENU_PPAGE ||
          cmd == KEY_CMD_MENU_FIRST || cmd == KEY_CMD_MENU_LAST)
      {
...
void iface_set_dir_content(const enum iface_menu iface_menu,
                           ...
                           const lists_t_strs *playlists)
{
  main_win_set_dir_content(&main_win, iface_menu, files, dirs, playlists);
  info_win_set_files_time(&info_win,
                          main_win_get_files_time(&main_win, iface_menu),
                          main_win_is_time_for_all(&main_win, iface_menu));

  iface_show_num_files(plist_count(files) + (dirs ? lists_strs_size(dirs) : 0) +
    ...
    if (m->type == MENU_DIR || m->type == MENU_PLAYLIST)
    {
           ...
           mi = mi->next, item_num += 1)
      {
        ...
        else if (mi->type == F_PLAYLIST)
        {
          ...
          menu_item_set_attr_sel(mi,
                                 get_color(CLR_MENU_ITEM_PLAYLIST_SELECTED));
        }
        else
        {
...
void iface_update_dir_content(const enum iface_menu iface_menu,
                              ...
                              const lists_t_strs *playlists)
{
  main_win_update_dir_content(&main_win, iface_menu, files, dirs, playlists);
  info_win_set_files_time(&info_win,
                          main_win_get_files_time(&main_win, iface_menu),
                          main_win_is_time_for_all(&main_win, iface_menu));
...
void iface_set_title(const enum iface_menu menu, const char *title)
{
  ...
  {
    main_win_set_title(
        &main_win, menu == IFACE_MENU_DIR ? MENU_DIR : MENU_PLAYLIST, title);
  }
  iface_refresh_screen();
}
...
int iface_in_dir_menu() { return main_win_in_dir_menu(&main_win); }

/* Return a non zero value if the playlist menu is currently selected. */
int iface_in_plist_menu() { return main_win_in_plist_menu(&main_win); }
...
int iface_in_entry() { return info_win_in_entry(&info_win); }
...
int iface_in_help() { return main_win_in_help(&main_win); }
...
void iface_temporary_exit() { endwin(); }
...
static void update_queue_position(struct plist *playlist,
                                  struct plist *dir_list, const char *file,
                                  const int pos)
{
  int i;
  ...
  if (playlist && (i = plist_find_fname(playlist, file)) != -1)
  {
    ...
    main_win_update_item(&main_win, IFACE_MENU_PLIST, playlist, i);
  }

  if (dir_list && (i = plist_find_fname(dir_list, file)) != -1)
  {
    dir_list->items[i].queue_pos = pos;
    main_win_update_item(&main_win, IFACE_MENU_DIR, dir_list, i);
  }
}
