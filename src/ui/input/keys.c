...
static struct command commands[] = {
    ...
    {KEY_CMD_PREVIOUS,
     ...
     {'b', -1},
     1},
    {KEY_CMD_PAUSE, "pause", "Pause", CON_MENU, {'p', ' ', -1}, 2},
    ...
    {KEY_CMD_SEEK_BACKWARD,
     ...
     {KEY_LEFT, -1},
     1},
    {KEY_CMD_HELP, "help", "Show the help screen", CON_MENU, {'h', '?', -1}, 2},
    ...
    {KEY_CMD_TOGGLE_SHOW_FORMAT,
     ...
     {CTRL('f'), -1},
     1},
    {KEY_CMD_GO_URL, "go_url", "Play from the URL", CON_MENU, {'o', -1}, 1},
    ...
    {KEY_CMD_GO_DIR,
     ...
     {'i', -1},
     1},
    {KEY_CMD_GO_DIR_UP, "go_up", "Go to '..'", CON_MENU, {'U', -1}, 1},
    {KEY_CMD_CANCEL,
     "cancel",
     "Exit from an entry",
     CON_ENTRY,
    ...
    {KEY_CMD_SEEK_BACKWARD_5,
     ...
     {'[', -1},
     1},
    {KEY_CMD_VOLUME_0, "volume_0", "Set volume to 0%", CON_MENU, {-1}, 0},
    {KEY_CMD_VOLUME_10,
     "volume_10",
     "Set volume to 10%",
     CON_MENU,
...
static struct special_keys
{
  char *name;
  int key;
} special_keys[] = {{"DOWN", KEY_DOWN},       {"UP", KEY_UP},
                    {"LEFT", KEY_LEFT},       {"RIGHT", KEY_RIGHT},
                    {"HOME", KEY_HOME},       {"BACKSPACE", KEY_BACKSPACE},
                    {"DEL", KEY_DC},          {"INS", KEY_IC},
                    {"ENTER", '\n'},          {"PAGE_UP", KEY_PPAGE},
                    {"PAGE_DOWN", KEY_NPAGE}, {"TAB", '\t'},
                    {"END", KEY_END},         {"KEYPAD_CENTER", KEY_B2},
                    {"SPACE", ' '},           {"ESCAPE", KEY_ESCAPE},
                    {"F1", KEY_F(1)},         {"F2", KEY_F(2)},
                    {"F3", KEY_F(3)},         {"F4", KEY_F(4)},
                    {"F5", KEY_F(5)},         {"F6", KEY_F(6)},
                    {"F7", KEY_F(7)},         {"F8", KEY_F(8)},
...
static void add_key(const int line_num, size_t cmd_ix, const char *key_symbol)
{
  ...
  for (i = commands[cmd_ix].default_keys; commands[cmd_ix].keys[i] != -1;
       i += 1)
  {
