...
#define nonblocking(fn, result, sock, buf, len)                                \
  ...
  {                                                                            \
    ...
    if (flags == -1)                                                           \
      fatal("Getting flags for socket failed: %s", xstrerror(errno));          \
    flags |= O_NONBLOCK;                                                       \
    if (fcntl(sock, F_SETFL, O_NONBLOCK) == -1)                                \
      fatal("Setting O_NONBLOCK for the socket failed: %s", xstrerror(errno)); \
...
void event_push(struct event_queue *q, const int event, void *data)
{
  ...
  else
  {
    ...
    q->tail->next = (struct event *)xmalloc(sizeof(struct event));
    q->tail = q->tail->next;
    q->tail->next = NULL;
    q->tail->type = event;
    q->tail->data = data;
...
void free_event_data(const int type, void *data)
{
  ...
  else if (type == EV_STATUS_MSG || type == EV_SRV_ERROR || type == EV_QUEUE_DEL)
  {
    free(data);
  }
...
static struct packet_buf *make_event_packet(const struct event *e)
{
  ...
  if (e->type == EV_QUEUE_DEL ||
      e->type == EV_SRV_ERROR || e->type == EV_STATUS_MSG)
  {
    assert(e->data != NULL);
    packet_buf_add_str(b, e->data);
  }
  else if (e->type == EV_QUEUE_ADD)
  {
    assert(e->data != NULL);
